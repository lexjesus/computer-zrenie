...
void MainWindow::openCamera()
{
    ...
    connect(capturer, &CaptureThread::updateStatus, this, &MainWindow::showStatus);
    capturer->start();
...
void MainWindow::showStatus(float fps, cv::Scalar cv_arr_mean, cv::Scalar arr_mean, cv::Scalar cv_arr_std_dev, cv::Scalar arr_std_dev){
    mainStatusLabel->setText(QString("FPS: %1 , cv_arr_mean: (%2, %3, %4), arr_mean: (%5, %6, %7), cv_arr_std_dev: (%8, %9, %10), arr_std_dev: (%11, %12, %13)")
                             .arg(int(fps))
                             .arg(int(cv_arr_mean[0])).arg(int(cv_arr_mean[1])).arg(int(cv_arr_mean[2]))
                             .arg(int(arr_mean[0])).arg(int(arr_mean[1])).arg(int(arr_mean[2]))
                             .arg(int(cv_arr_std_dev[0])).arg(int(cv_arr_std_dev[1])).arg(int(cv_arr_std_dev[2]))
                             .arg(int(arr_std_dev[0])).arg(int(arr_std_dev[1])).arg(int(arr_std_dev[2]))
            );
}
